...
describe('ProtectedRoute Component', () => {
  ...
  it('renders children when user is authenticated and authorized', () => {
    vi.spyOn(AuthContextModule, 'useAuth').mockReturnValue({
      user: { id: 1, email: 'test@example.com', fullName: 'Test User', role: UserRole.Admin, isTotpEnabled: false },
      token: 'test-token',
      isLoading: false,
      login: vi.fn(),
      logout: vi.fn(),
  ...
  it('shows 403 page when user lacks required role', () => {
    vi.spyOn(AuthContextModule, 'useAuth').mockReturnValue({
      user: { id: 2, email: 'buyer@example.com', fullName: 'Buyer User', role: UserRole.Buyer, isTotpEnabled: false },
      token: 'test-token',
      isLoading: false,
      login: vi.fn(),
      logout: vi.fn(),
